...
public class FileService extends BaseService<FileInfo, Long, FileInfoRepository> {
    ...
    @Override
    @Cacheable(value = Cache.APPLICATION, key = "'" + CacheKey.FILE_INFO + "'+#id", unless = "#result == null")
    ...
    @Cacheable(value = Cache.APPLICATION, key = "'" + CacheKey.FILE_INFO + "'+#fileName", unless = "#result == null")
    public FileInfo getByName(String fileName) {
        FileInfo fileInfo = null;
        List<FileInfo> fileInfos = fileInfoRepository.findAllByOriginalFileName(fileName);
        if (fileInfos.size() != 0) {
            fileInfo = fileInfos.get(0);
            fileInfo.setAblatePath(configCache.get(ConfigKeyEnum.SYSTEM_FILE_UPLOAD_PATH.getValue()) + File.separator + fileInfo.getRealFileName());
        } else {
            fileInfo = fileInfoRepository.findAllByOriginalFileName("404.png").get(0);
            fileInfo.setAblatePath(configCache.get(ConfigKeyEnum.SYSTEM_FILE_UPLOAD_PATH.getValue()) + File.separator + fileInfo.getRealFileName());
        }
        return fileInfo;
    }
